...
{$IFNDEF BCB}
{$IFDEF COMPILER5}
{ These functions simply call their rm_JvVCL5Utils equivalents }
...
// Math
type
  TValueSign = rm_JvVCL5Utils.TValueSign;
...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$RCSfile$';
    Revision: '$Revision: 10302 $';
    Date: '$Date: 2006-02-25 09:51:39 -0800 (Sat, 25 Feb 2006) $';
    LogPath: 'JVCL\run'
  );
{$ENDIF UNITVERSIONING}
...
resourcestring
  // (p3) duplicated from rm_JvConsts since this unit should not rely on JVCL at all
  RsEPropertyNotExists = 'Property "%s" does not exist';
  RsEInvalidPropertyType = 'Property "%s" has invalid type';
  RsEPivotLessThanZero = 'rm_JvJCLUtils.MakeYear4Digit: Pivot < 0';
...
{$ENDIF VCL}
  ...
  // (p3) duplicated from rm_JvTypes to avoid JVCL dependencies
  {$IFDEF VCL}
type
...
function TryStrToInt(const S: string; out Value: Integer): Boolean;
begin
  Result := rm_JvVCL5Utils.TryStrToInt(S, Value);
end;

function TryStrToDateTime(const S: string; out Date: TDateTime): Boolean;
begin
  Result := rm_JvVCL5Utils.TryStrToDateTime(S, Date);
end;

function StrToDateTimeDef(const S: string; Default: TDateTime): TDateTime;
begin
  Result := rm_JvVCL5Utils.StrToDateTimeDef(S, Default);
end;

// function StrToFloatDef(const Str: string; Default: Extended): Extended;
procedure RaiseLastOSError;
begin
  rm_JvVCL5Utils.RaiseLastOSError;
end;

function IncludeTrailingPathDelimiter(const APath: string): string;
begin
  Result := rm_JvVCL5Utils.IncludeTrailingPathDelimiter(APath);
end;

function ExcludeTrailingPathDelimiter(const APath: string): string;
begin
  Result := rm_JvVCL5Utils.ExcludeTrailingPathDelimiter(APath);
end;

function DirectoryExists(const Name: string): Boolean;
begin
  Result := rm_JvVCL5Utils.DirectoryExists(Name);
end;

function ForceDirectories(Dir: string): Boolean;
begin
  Result := rm_JvVCL5Utils.ForceDirectories(Dir);
end;

function SameFileName(const FN1, FN2: string): Boolean;
begin
  Result := rm_JvVCL5Utils.SameFileName(FN1, FN2);
end;

function WideCompareText(const S1, S2: WideString): Integer;
begin
  Result := rm_JvVCL5Utils.WideCompareText(S1, S2);
end;

function WideUpperCase(const S: WideString): WideString;
begin
  Result := rm_JvVCL5Utils.WideUpperCase(S);
end;

function WideLowerCase(const S: WideString): WideString;
begin
  Result := rm_JvVCL5Utils.WideLowerCase(S);
end;

function CompareDateTime(const A, B: TDateTime): Integer;
begin
  Result := rm_JvVCL5Utils.CompareDateTime(A, B);
end;

// StrUtils
function AnsiStartsText(const SubText, Text: string): Boolean;
begin
  Result := rm_JvVCL5Utils.AnsiStartsText(SubText, Text);
end;

function AnsiEndsText(const SubText, Text: string): Boolean;
begin
  Result := rm_JvVCL5Utils.AnsiEndsText(SubText, Text);
end;

function AnsiStartsStr(const SubStr, Str: string): Boolean;
begin
  Result := rm_JvVCL5Utils.AnsiStartsStr(SubStr, Str);
end;

function AnsiEndsStr(const SubStr, Str: string): Boolean;
begin
  Result := rm_JvVCL5Utils.AnsiEndsStr(SubStr, Str);
end;

// Variants
function VarIsStr(const V: Variant): Boolean;
begin
  Result := rm_JvVCL5Utils.VarIsStr(V);
end;
