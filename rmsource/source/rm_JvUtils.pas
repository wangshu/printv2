...
//  { returns date relative to current date: 'äâ?äíÿ íàçà? }
  ...
  case Length(NumStr) of
    ...
    5:
      begin
        ...
        I := Pos(' äâ?, StrVsp2);
        if Pos(' äâ?, StrVsp2) = I then
          I := 0;
        if I <> 0 then
          StrVsp2[I + 3] := 'e';
        I := Pos(' îäèí', StrVsp2);
        if Pos(' îäèí?, StrVsp2) = I then
          I := 0;
        if I <> 0 then
        begin
          StrVsp2[I + 3] := '?;
      ...
      begin
        ...
        case NumStr[1] of
          '1':
            StrVsp := 'îäèí ìèëëèî?;
          '2'..'4':
            StrVsp := StrVsp + ' ìèëëèîíà';
          '5'..'9':
            StrVsp := StrVsp + ' ìèëëèîíî?;
        end;
...
function Win2Koi(const S: string): string;
const
  W = 'àáâãäå¸æçèéêëìíîïðñòóôõ÷öøùüûúýþÿÀÁÂÃÄÅ¨ÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕ×ÖØÙÜÛÚÝÞ?';
  K = 'ÁÂ×ÇÄÅ£ÖÚÉÊËÌÍÎÏÐÒÓÔÕÆÈÞÃÛÝØÙßÜÀÑáâ÷çäå³öúéêëìíîïðòóôõæèþãûýøùÿüà?';
var
  I, J: Integer;
begin
  Result := S;
