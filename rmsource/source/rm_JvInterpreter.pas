...
begin
  if DllInstance > 0 then
    Result := rm_JvInterpreter.CallDllIns(DllInstance, FunctionName, Args, FunctionDesc.FParamTypes,
      FunctionDesc.ResTyp)
  else
    Result := rm_JvInterpreter.CallDll(DllName, FunctionName, Args, FunctionDesc.FParamTypes,
      FunctionDesc.ResTyp)
end;
...
var
  rm_JvInterpreterConst: TJvInterpreterConst;
begin
  rm_JvInterpreterConst := TJvInterpreterConst.Create;
  rm_JvInterpreterConst.Identifier := AIdentifier;
  rm_JvInterpreterConst.Value := AValue;
  rm_JvInterpreterConst.Data := AData;
  rm_JvInterpreterConst.UnitName := AUnitName;
  FConstList.Add(rm_JvInterpreterConst);
  FSorted := False; 
end;
...
var
  ...
  begin
    ...
    if Result then
    begin
      rm_JvInterpreterConst := TJvInterpreterConst(FConstList[I]);
      CheckAction(Expression, Args, rm_JvInterpreterConst.Data);
      Value := rm_JvInterpreterConst.Value;
    end;
  end;
...
begin
  ...
  begin
    ...
    ArgCount := Args.Count;
    NamedArgCount := 0; { named args not supported by rm_JvInterpreter }
  end;
  Names := Identifier + #00;
  Ptr := 0;
  TypePtr := 0;
...
begin
  ...
  {$IFDEF JvInterpreter_DEBUG}
  if ObjCount <> 0 then
    Windows.MessageBox(0, PChar('Memory leak in rm_JvInterpreter.pas'#10 +
      'ObjCount = ' + IntToStr(ObjCount)),
      'rm_JvInterpreter Internal Error', MB_ICONERROR);
  {$ENDIF JvInterpreter_DEBUG}
end;
