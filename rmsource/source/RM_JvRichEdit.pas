...
type
  ...
  private
    RichEdit: TRMCustomUnicodeRichEdit {TNT-ALLOW TCustomRichEdit};
    procedure EnableChange(const Value: Boolean);
  protected
...
var
  SelChange: TNotifyEvent;
begin
  SelChange := TRMCustomUnicodeRichEdit(RichEdit).OnSelectionChange;
  TRMCustomUnicodeRichEdit(RichEdit).OnSelectionChange := nil;
  try
    inherited;
  finally
    TRMCustomUnicodeRichEdit(RichEdit).OnSelectionChange := SelChange;
  end;
end;
...
type TAccessCustomRichEdit = class(TJvCustomRichEdit);
...
constructor TRMCustomUnicodeRichEdit.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TRMCustomUnicodeRichEdit.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
  if WordWrap then
...
procedure TRMCustomUnicodeRichEdit.CreateWindowHandle(const Params: TCreateParams);
begin
...
procedure TRMCustomUnicodeRichEdit.CreateWnd;
const
...
procedure TRMCustomUnicodeRichEdit.DefineProperties(Filer: TFiler);
begin
  inherited;
  TntPersistent_AfterInherited_DefineProperties(Filer, Self);
end;

destructor TRMCustomUnicodeRichEdit.Destroy;
begin
  FreeAndNil(FRichEditStrings);
  inherited;
end;

function TRMCustomUnicodeRichEdit.LineBreakStyle: TTntTextLineBreakStyle;
begin
  if Win32PlatformIsUnicode and IsRichEdit20Available then
    Result := tlbsCR
  else
...
procedure TRMCustomUnicodeRichEdit.SetRichEditStrings(const Value: TTntStrings);
begin
  FRichEditStrings.Assign(Value);
end;

function TRMCustomUnicodeRichEdit.GetSelText: string {TNT-ALLOW string};
begin
  Result := GetWideSelText;
end;

function TRMCustomUnicodeRichEdit.GetWideSelText: WideString;
var
  CharRange: TCharRange;
  Length: Integer;
begin
...
procedure TRMCustomUnicodeRichEdit.SetRTFText(Flags: DWORD; const Value: AnsiString);
const
...
procedure TRMCustomUnicodeRichEdit.SetWideSelText(const Value: WideString);
const
  ST_SELECTION = 2;
begin
...
function TRMCustomUnicodeRichEdit.GetText: WideString;
begin
...
procedure TRMCustomUnicodeRichEdit.SetText(const Value: WideString);
const
  ST_DEFAULT = 0;
begin
  if Win32PlatformIsUnicode and IsRichEdit20Available and IsRTF(Value) then begin
...
function TRMCustomUnicodeRichEdit.IsHintStored: Boolean;
begin
  Result := TntControl_IsHintStored(Self);
end;

function TRMCustomUnicodeRichEdit.GetHint: WideString;
begin
  Result := TntControl_GetHint(Self);
end;

procedure TRMCustomUnicodeRichEdit.SetHint(const Value: WideString);
begin
  TntControl_SetHint(Self, Value);
end;

procedure TRMCustomUnicodeRichEdit.WMGetTextLength(var Message: TWMGetTextLength);
begin
...
procedure TRMCustomUnicodeRichEdit.Print(const Caption: string {TNT-ALLOW string});
begin
  if (LineBreakStyle <> tlbsCRLF) then
...
function TRMCustomUnicodeRichEdit.CharPosToGet(RawWin32CharPos: Integer): Integer;
begin
  Result := EmulatedCharPos(RawWin32CharPos);
end;

function TRMCustomUnicodeRichEdit.CharPosToSet(EmulatedCharPos: Integer): Integer;
begin
  Result := RawWin32CharPos(EmulatedCharPos);
end;
{$WARN SYMBOL_DEPRECATED ON}

function TRMCustomUnicodeRichEdit.EmulatedCharPos(RawWin32CharPos: Integer): Integer;
var
  i: Integer;
  ThisLine: Integer;
  CharCount: Integer;
...
function TRMCustomUnicodeRichEdit.RawWin32CharPos(EmulatedCharPos: Integer): Integer;
var
  Line: Integer;
  NumLineBreaks: Integer;
  CharCount: Integer;
...
function TRMCustomUnicodeRichEdit.FindText(const SearchStr: WideString;
  StartPos, Length: Integer; Options: TRichSearchTypes): Integer;
const
...
function TRMCustomUnicodeRichEdit.GetSelStart: Integer;
begin
  Result := TntCustomEdit_GetSelStart(Self);
  Result := EmulatedCharPos(Result);
end;

procedure TRMCustomUnicodeRichEdit.SetSelStart(const Value: Integer);
begin
  TntCustomEdit_SetSelStart(Self, RawWin32CharPos(Value));
end;

function TRMCustomUnicodeRichEdit.GetSelLength: Integer;
var
  CharRange: TCharRange;
begin
  if (LineBreakStyle = tlbsCRLF) then
...
procedure TRMCustomUnicodeRichEdit.SetSelLength(const Value: Integer);
var
  StartPos: Integer;
  SelEnd: Integer;
begin
